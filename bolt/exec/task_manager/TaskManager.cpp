...
std::shared_ptr<WrappedTask> TaskManager::createTask(
  ...
  std::shared_ptr<WrappedTask> boltTask;
  // Serialize creation and start with shutdown so a task cannot start after
  // shutdown has taken its snapshot of outstanding work.
  ...
  BOLT_CHECK(
      ...
      "Task {} already exists",
      taskId);
  boltTask = std::make_shared<WrappedTask>(
      outputBufferPool_,
      arrowOptions,
      fieldNames,
      bufferedBytes,
